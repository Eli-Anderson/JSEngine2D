...
class Vector2 {
    ...
    div(k) {
        if (k instanceof Vector2) {
            if (k.x === 0) console.warn("Divided a Vector2's x component by 0");
            if (k.y === 0) console.warn("Divided a Vector2's x component by 0");
            this.x /= k.x;
            this.y /= k.y;
        } else if (!isNaN(k)) {
            if (k === 0) console.warn("Divided a Vector2 by 0");
            this.x /= k;
            this.y /= k;
        }
        return this;
    ...
    static add(a, b) {
        ...
        console.error(
            "Attempted to call Vector2.add without a Vector2 as a parameter"
        );
        return new Vector2({ x, y });
    }
    ...
    static mult(a, b) {
        ...
        if (a instanceof Vector2) {
            if (b instanceof Vector2) {
                ...
                console.error(
                    "The second argument of Vector2.mult should be a Vector or a Number",
                    b
                );
            }
        } else {
            console.error(
                "The first argument of Vector2.mult should be a Vector2",
                a
            );
        }
    ...
    static div(a, b) {
        ...
        let y = 0;
        if (b instanceof Vector2) {
            if (b.x === 0) console.warn("Divided a Vector2's x component by 0");
            if (b.y === 0) console.warn("Divided a Vector2's y component by 0");
            x = a.x / b.x;
            y = a.y / b.y;
        } else if (isNaN(b) === false) {
            if (b === 0) console.warn("Divided a Vector2 by 0");
            x = a.x / b;
            y = a.y / b;
        }
        return new Vector2({ x, y });
...
class Vector3 extends Vector2 {
    ...
    /**
     * Check whether or not this Vector3 is equal to the passed in Vector3. This does straight
     * === checks on each of the x, y, and z components.
     *
     * @param      {Vector3}  other   The other Vector3 to compare with
     * @return     {boolean}  { Returns true if the Vector3s are equal, false otherwise }
     */
    equals(other) {
...
class Rect extends Vector2 {
    ...
    get edges() {
        return {
            top: new Line({
                pointA: new Vector2({ x: this.x, y: this.y }),
                pointB: new Vector2({ x: this.right, y: this.y })
            }),

            right: new Line({
                pointA: new Vector2({ x: this.right, y: this.y }),
                pointB: new Vector2({ x: this.right, y: this.bottom })
            }),

            bottom: new Line({
                pointA: new Vector2({ x: this.x, y: this.bottom }),
                pointB: new Vector2({ x: this.right, y: this.bottom })
            }),
...
class Circle extends Vector2 {
    ...
    equals(circle) {
        return (
            this.x === circle.x && this.y === circle.y && this.r === circle.r
        );
    }
    get top() {
...
class Transform extends Vector3 {
    ...
    constructor({ x = 0, y = 0, z = 0, width = 0, height = 0, center }) {
        ...
        this._defaultRotation = Vector2.angleBetween(
            Vector2.zero,
            this._forward
        );
    }
    ...
    lerpRotation(angle, t) {
        ...
        if (this.rotation - a > Math.PI) {
            this.rotation = Math.abs(
                (this.rotation + (a - (this.rotation - PI2)) * t) % PI2
            );
            ...
            this.rotation = Math.abs(
                (this.rotation + (a - this.rotation) * t) % PI2
            );
        }
    }
...
class Container {
    ...
    add(child) {
        if (child instanceof Component) {
            console.error(
                "Attempted to add a component as a child... Did you mean to use `attach`?"
            );
        } else if (child instanceof Container === false) {
            console.error(
                "You can only add objects of type Container to this container",
                child
            );
    ...
    moveTo(vector3) {
        ...
        for (const child of this.flattened) {
            child.transform.set(
                Vector3.add(child.transform, positionDifference)
            );
        }
    }

    moveCenterTo(vector3) {
        let adjustedPos = Vector2.sub(
            vector3,
            new Vector2({
    ...
    moveCenterTowards(destination, s) {
        let norm = Vector2.sub(destination, this.transform.rect.center)
            .normalized;
        this.move(norm.mult(s));
    }
...
class GameObject extends Container {
    constructor({ transform, isUI = false }) {
        ...
        if (transform === undefined) {
            console.warn(
                "GameObject created without a Transform. Using the default " +
                    "Transform. To use the default Transform explicitly, use " +
                    'new GameObject({"transform": new Transform({})})',
                this
            );
        }
        this.isUI = isUI;
    ...
    set rotation(radians) {
        ...
        for (const child of this.children) {
            let angleToChild = Vector2.angleBetween(
                child.transform.rect.center,
                this.transform.rect.center
            );
            let distanceToChild = Vector2.sub(
                this.transform.rect.center,
                child.transform.rect.center
            ).magnitude;
            ...
            child.moveCenterTo(
                Vector2.add(this.transform.rect.center, newRelativePosition)
            );
        }
    }
    ...
    rotate(radians) {
        ...
        this.transform.rotation =
            (this.transform.rotation + radians) % (Math.PI * 2);
        for (let child of this._flattened) {
            child.transform.rotation =
                (child.transform.rotation + radians) % (Math.PI * 2);
        }
    }
    ...
    lerpRotateTowards(position, t) {
        ...
        let angle =
            Vector2.angleBetween(this.transform.rect.center, position) +
    ...
    rotateTowards(position, maxDegrees) {
        ...
        if (this.rotation - angle > Math.PI) {
            this.rotation = Math.abs(
                (this.rotation + (angle - (this.rotation - PI2))) % PI2
            );
            ...
            this.rotation = Math.abs(
                (this.rotation + (angle - this.rotation)) % PI2
            );
        }
    }
    ...
    attach(component) {
        for (const c of this.components) {
            if (c instanceof component.constructor) {
                console.error(
                    "GameObject already has a component of type: " +
...
class Scene {
    ...
    add(child) {
        if (child instanceof GameObject) {
            if (child !== this && child.parent === null) {
                // child is not this, child is an orphan
                child._parent = this;
                this.children.push(child);
                this._addToFlattened(child);
    ...
    hasDescendant(child, children = this.children) {
		...
		*/
        ...
        if (children[0].parent.hasChild(child)) return true;
        for (const c of this.children) {
            if (c.children) {
                // make sure it is able to have children
                if (c.children.length > 0) {
                    if (this.hasDescendant(child, c.children)) {
                        // if it does, run this again with this child as the parent
                        return true;
                    }
                }
            }
        ...
        if (type === Input._MOUSEMOVE) {
            ...
            for (const component of this._inputComponents) {
                if (
                    ...
                    if (
                        topMost === null ||
                        component.gameObject.transform.z >
                            topMost.gameObject.transform.z
                    )
                        topMost = component;
                }
            }
            if (topMost !== null) topMost._onClick(Input.mouse.rel);
            ...
            if (topMost !== null) topMost._onDblClick(Input.mouse.rel);
            ...
            if (topMost !== null) topMost._onRightClick(Input.mouse.rel);
        }
    }
    ...
    static clear(player) {
        ...
        if (Scene.current.mainCamera) {
            ...
            player.context.fillRect(
                0,
                0,
                Scene.current.mainCamera.viewport.width,
    ...
    static update(dt) {
        if (Scene.current === null) return;
        Scene.current.flattened.forEach(gameObject => {
...
class InputComponent extends Component {
    ...
    remove() {
        if (this.gameObject) {
            Scene.getScene(this.gameObject._sceneKey).removeInputComponent(
                this
            );
        }
        super.remove();
...
class Animator extends Component {
    ...
    set animation(key) {
        ...
        this.gameObject.getComponent(
            SpriteRenderer
        ).sprite = this.animation.frames[this.frame];
    }
...
class Camera extends Component {
	...
	*/
    constructor({
        viewport = new Rect({ x: 0, y: 0, width: 720, height: 480 }),
                ...
                if (component.draw && component instanceof Camera === false) {
                    if (el.isUI) {
                        ...
                        if (
                            ...
                            let screenPos = this.worldToScreenPosition(
                                el.transform
                            );
	...
	*/
    ...
    static createEventListeners(target) {
        ...
        target.addEventListener("keyup", e =>
            Input.handleInput(e.key, Input.KEYUP)
        );

        target.addEventListener("mousemove", function(e) {
            //TODO: For some reason the left-most edge of the canvas shows up as > 0 in e.clientX
            Input.mouse.abs.x =
                e.clientX - (target.offsetLeft ? target.offsetLeft : 0);
            Input.mouse.abs.y =
                e.clientY - (target.offsetTop ? target.offsetTop : 0);
            if (
                Game.instance !== null &&
                Scene.current &&
                Scene.current.mainCamera
            ) {
                // since the Camera scale is held separate from the Player scale, we need to account
                // for both of them multiplicatively.
                let actualScale = Vector2.mult(
                    Game.instance.player.scale,
                    Scene.current.mainCamera.scale
                );
...
Input.SPACE = " ";
Input.keys = {
    q: new InputKey({ name: "q", upper: "Q" }),
    w: new InputKey({ name: "w", upper: "W" }),
    e: new InputKey({ name: "e", upper: "E" }),
    r: new InputKey({ name: "r", upper: "R" }),
    t: new InputKey({ name: "t", upper: "T" }),
    y: new InputKey({ name: "y", upper: "Y" }),
    u: new InputKey({ name: "u", upper: "U" }),
    i: new InputKey({ name: "i", upper: "I" }),
    o: new InputKey({ name: "o", upper: "O" }),
    p: new InputKey({ name: "p", upper: "P" }),
    a: new InputKey({ name: "a", upper: "A" }),
    s: new InputKey({ name: "s", upper: "S" }),
    d: new InputKey({ name: "d", upper: "D" }),
    f: new InputKey({ name: "f", upper: "F" }),
    g: new InputKey({ name: "g", upper: "G" }),
    h: new InputKey({ name: "h", upper: "H" }),
    j: new InputKey({ name: "j", upper: "J" }),
    k: new InputKey({ name: "k", upper: "K" }),
    l: new InputKey({ name: "l", upper: "L" }),
    ";": new InputKey({ name: ";", upper: ":" }),
    "'": new InputKey({ name: "'", upper: '"' }),
    z: new InputKey({ name: "z", upper: "Z" }),
    x: new InputKey({ name: "x", upper: "X" }),
    c: new InputKey({ name: "c", upper: "C" }),
    v: new InputKey({ name: "v", upper: "V" }),
    b: new InputKey({ name: "b", upper: "B" }),
    n: new InputKey({ name: "n", upper: "N" }),
    m: new InputKey({ name: "m", upper: "M" }),
    ",": new InputKey({ name: ",", upper: "<" }),
    ".": new InputKey({ name: ".", upper: ">" }),
    "/": new InputKey({ name: "/", upper: "?" }),
    "1": new InputKey({ name: "1", upper: "!" }),
    "2": new InputKey({ name: "2", upper: "@" }),
    "3": new InputKey({ name: "3", upper: "#" }),
    "4": new InputKey({ name: "4", upper: "$" }),
    "5": new InputKey({ name: "5", upper: "%" }),
    "6": new InputKey({ name: "6", upper: "^" }),
    "7": new InputKey({ name: "7", upper: "&" }),
    "8": new InputKey({ name: "8", upper: "*" }),
    "9": new InputKey({ name: "9", upper: "(" }),
    "0": new InputKey({ name: "0", upper: ")" }),
    "-": new InputKey({ name: "-", upper: "_" }),
    "=": new InputKey({ name: "=", upper: "+" }),
    "`": new InputKey({ name: "`", upper: "~" }),
...
class RigidBody extends Component {
    ...
    remove() {
        super.remove();
        // remove from Input
        PhysicsEngine.rigidbodies.splice(
            PhysicsEngine.rigidbodies.indexOf(this),
            1
        );
    }
...
class Collider extends Component {
    ...
    get collisions() {
        ...
        for (let i = 0; i < this._collisions.length; i++) {
            ...
            if (
                coll.colliderB.gameObject !== null &&
                coll.colliderB.gameObject.enabled &&
                coll.colliderB.enabled &&
                coll.colliderA.gameObject.enabled &&
                coll.colliderA.enabled
            ) {
    ...
    get colliders() {
        ...
        for (let i = 0; i < this._colliders.length; i++) {
            ...
            if (
                typeof coll.gameObject !== "undefined" &&
                coll.enabled &&
                coll.gameObject.enabled
            ) {
    ...
    remove() {
        ...
        PhysicsEngine.colliders.splice(
            PhysicsEngine.colliders.indexOf(this),
            1
        );
        super.remove();
    }
    ...
    checkCollisionWith(otherCollider) {
        if (
            this.layer === otherCollider.layer ||
            this.layer === Collider.LAYER_ALL ||
            otherCollider.layer === Collider.LAYER_ALL
        ) {
            let result = PhysicsEngine.checkCollision(
                this.bound,
                otherCollider.bound
            );
    ...
    _onExit(collision) {
        ...
        for (let i = 0; i < this.collisions.length; i++) {
            if (
                this.collisions[i].colliderA === collision.colliderA &&
...
class SpriteRenderer extends Component {
    ...
    draw(context, position) {
        if (this.gameObject.enabled && this.enabled && this.sprite) {
            ...
            if (t.rotation) {
                context.translate(
                    position.x + t.width / 2,
                    position.y + t.height / 2
                );
...
class ImageResource extends Resource {
    constructor({ path }) {
        ...
        this.image.onload = ev => {
            this.loaded = true;
            this.onLoad(ev); // called on Loader
        };
        this.image.onerror = ev => {
            this.failed = true;
            this.onError(ev); // called on Loader
        };
        this.image.src = path;
    }
}
...
class Loader {
    ...
    add(key, resource) {
        if (resource instanceof Resource === false) {
            console.error(
                "Loader only accepts objects that extend the Resource class"
            );
        } else {
            ...
            if (resource.loaded) {
                ...
                resource.onLoad = () => {
                    ...
                    if (
                        this.size != null &&
                        Object.keys(this.loadedResources).length === this.size
                    ) {
...
class Sprite {
    ...
    set cropRect(cropRect) {
        if (cropRect.constructor !== Rect)
            console.error(
                "SpriteRenderer cropRect property set to object of type other than Rect"
            );
    ...
    static setSprite(key, sprite) {
        if (key.constructor !== String) {
            console.error(
                "Attempted to store a Sprite with an invalid key... Must be a String"
            );
        } else if (sprite instanceof Sprite === false) {
            console.error(
                "Sprite attempted to be stored was not of the right type... Must be a Sprite object"
            );
        }
    ...
    static getSprite(key) {
        if (key.constructor !== String) {
            console.error(
                "Attempted to retrieve a Sprite with an invalid key... Must be a String"
            );
        }
...
class TextRenderer extends Component {
    constructor({ text = "", font = Font.default }) {
        super();
        this._lines = [];
        this._splitLines = [];
        this.noStyleText = "";
    ...
    set text(text) {
        ...
        if (openIndex >= 0 && closeIndex > openIndex) {
            this._splitLines.push([
                "",
                this._text.substring(0, openIndex).split("\n")
            ]);
        ...
        while (openIndex >= 0 && closeIndex > openIndex) {
            ...
            this._splitLines.push([
                ...
                this._text
                    .substring(
                        closeIndex + 1,
                        nextOpenIndex >= 0 ? nextOpenIndex : this._text.length
                    )
                    .split("\n")
            ]);
            this.noStyleText += this._text.substring(
                closeIndex + 1,
                nextOpenIndex >= 0 ? nextOpenIndex : this._text.length
            );
    ...
    draw(context, transform) {
        if (this.gameObject.enabled && this.enabled) {
            ...
            if (this.font.vAlignment === Font.TOP) {
                ...
                vAlign =
                    transform.height / 2 +
                    this.font.size * this.font.sizeHeightRatio;
            ...
            for (const line of this._splitLines) {
                if (line[0].length > 2) {
                    ...
                    for (let i = 0; i < args.length; i++) {
                        ...
                        if (arg instanceof Color) context.fillStyle = arg;
                        else if (
                            arg === Font.BOLD ||
                            arg === Font.ITALIC ||
                            arg === Font.NORMAL
                ...
                for (const index2 in line[1]) {
                    ...
                    if (this.font.hAlignment === Font.CENTERED) {
                        ...
                        hAlign -=
                            context.measureText(splitText[lineIndex]).width / 2;
                        hAlign += transform.width / 2;
                    } else if (this.font.hAlignment === Font.RIGHT) {
                        hAlign -= context.measureText(splitText[lineIndex])
                            .width;
                    ...
                    context.fillText(
                        line[1][index2],
                        transform.x + widthInLine + hAlign,
                        transform.y + vAlign + lineIndex * this.font.size
                    );
                    if (index2 === "0") {
                        widthInLine += context.measureText(line[1][index2])
                            .width;
                    }
                }
            }
	...
	*/
    static update(dt) {
        ...
        for (const coll1 of PhysicsEngine.colliders) {
            if (coll1.enabled === false || coll1.gameObject.enabled === false)
                continue;
            for (const coll2 of PhysicsEngine.colliders) {
                if (
                    coll1.enabled === false ||
                    coll1.gameObject.enabled === false
                )
                    break;

                if (
                    coll2.enabled &&
                    coll2.gameObject.enabled &&
                    coll1 !== coll2
                ) {
    ...
    static resolveImpulse(collision) {
        ...
        if (
            a.getComponent(RigidBody).isStatic &&
        ...
        let e = Math.min(
            a.getComponent(PhysicsCollider).bounciness,
        ...
        if (!bRb.isStatic) {
            ...
            let frictionVector = Vector2.mult(
                normal.normal,
                compositeSlickness
            );
    ...
    static resolvePosition(collision) {
        ...
        let invMassA =
            1 / collision.colliderA.gameObject.getComponent(RigidBody).mass;
        let invMassB =
            1 / collision.colliderB.gameObject.getComponent(RigidBody).mass;
        ...
        if (!collision.colliderA.gameObject.getComponent(RigidBody).isStatic) {
            collision.colliderA.gameObject.move(
                Vector2.mult(-invMassA * correction, collision.normal)
            );
        }
        if (!collision.colliderB.gameObject.getComponent(RigidBody).isStatic) {
            collision.colliderB.gameObject.move(
                Vector2.mult(invMassB * correction, collision.normal)
            );
        }
    }
    ...
    static checkCollision(objA, objB) {
        ...
        if (objA.constructor === Vector2) {
            ...
            if (objB.constructor === Vector2) return objA.equals(objB);
        }
    ...
    static collisionLC(line, circle) {
        ...
        let LEC = Math.sqrt(
            (E.x - C.x) * (E.x - C.x) + (E.y - C.y) * (E.y - C.y)
        );
    ...
    static collisionLL(lineA, lineB) {
        ...
        let s, t;
        s =
            (-dvA.y * (lineA.start.x - lineB.start.x) +
                dvA.x * (lineA.start.y - lineB.start.y)) /
            (-dvB.x * dvA.y + dvA.x * dvB.y);
        t =
            (dvB.x * (lineA.start.y - lineB.start.y) -
                dvB.y * (lineA.start.x - lineB.start.x)) /
            (-dvB.x * dvA.y + dvA.x * dvB.y);
        if (s >= 0 && s <= 1 && t >= 0 && t <= 1) {
            // collision detected
...
class Raycast {
    ...
    get rayEnd() {
        return Vector2.add(
            this.rayStart,
            Vector2.mult(this.rayDirection, this.rayLength)
        );
    }
    ...
    get contacts() {
        ...
        for (const coll of PhysicsEngine.colliders) {
            ...
            if (coll.bound instanceof Rect) {
                for (const edge of coll.bound.edges) {
                    let intersect = PhysicsEngine.collisionLL(
                        this.rayToLine,
                        edge
                    );
        ...
        if (rayCastPoints.length > 0) {
            rayCastPoints.sort((a, b) => {
                return (
                    Vector2.sub(this.rayStart, a.first).magnitudeSquared -
...
class Random {
    ...
    static sample(array, n) {
        ...
        if (array.length > 3) {
            ...
            // TODO: if array.length === 3 and n === 2 this may produce duplicates
            for (let i = 0; i < n; i++) {
    ...
    _update() {
        if (this.running) {
            ...
            if (!this.paused) {
                ...
                if (
                    this._physicsUpdateDelay === 0 ||
                    this.ticks % this._physicsUpdateDelay === 0
                ) {
...
class Player {
    ...
    scaleMult(scale) {
        if (scale.constructor !== Vector2) {
            console.error(
                "Player.scaleMult parameter 'scale' must be of type Vector2."
            );
        }
        if (scale.x != 0 && scale.y != 0) {
	...
	*/
    ...
    isMobile() {
        let mobileChecks = {
            ...
            any: function() {
                return (
                    mobileChecks.Android() ||
                    mobileChecks.BlackBerry() ||
                    mobileChecks.iOS() ||
                    mobileChecks.Opera() ||
                    mobileChecks.Windows()
                );
            }
        };
    ...
    rescale() {
        if (window.innerWidth * this.aspectRatio <= window.innerHeight) {
            ...
            this.size = new Vector2({
                x: window.innerWidth,
                y: window.innerWidth * this.aspectRatio
            });
            ...
            this.size = new Vector2({
                x: window.innerHeight / this.aspectRatio,
                y: window.innerHeight
            });
            // scale based on height instead of width
            this.canvas.style.left =
                (window.innerWidth - this.size.x) / 2 + "px";
...
class TextBox extends GameObject {
    ...
    constructor({
        ...
        this.maxLength = Math.floor(
            this.transform.width / (transform.height * (3 / 5))
        );
    ...
    update(dt) {
        ...
        for (let key in Input.downKeys) {
            ...
            if (this.timeHeld >= this.timeHoldPerChar) {
                this.timeHeld = 0.0;
                if (this.timeHoldPerChar > 0.05)
                    this.timeHoldPerChar *= this.timeHoldMultRate;
                ...
                if (key.length === 1) {
                    ...
                    if (tr.text.length < this.maxLength) {
                        if (Input.keys[key].down) {
                            tr.text =
                                tr.text.substring(0, this.cursorIndex) +
                                key +
                                tr.text.substring(
                                    this.cursorIndex,
                                    tr.text.length
...
class TileMap extends GameObject {
    constructor(transform, tileList) {
        ...
        this.buffer.name = "TileMapBackBuffer";
